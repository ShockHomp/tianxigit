...
//创建签约存order_no
//$fp_log_lyf = fopen(dirname(__FILE__) . '/withdraw_batch_sign.log', 'ab');
...
//);
if (empty($_POST)) {
    echo json_encode(array('bank_sign_up' => 1, 'msg' => '请用post请求,并传指定参数'),JSON_UNESCAPED_UNICODE);
    exit;
}
try {
...
function withdrawBatchSign($pdo)
{
    ...
    //签约查询
    $sql = 'select
           b.bank_num,b.id,b.phone,b.real_name,b.id_card,b.bank_sign_up,b.member_id,b.order_no
    ...
    if (empty($arrayAll)) {
        echo json_encode(array('bank_sign_up' => '00000009', 'msg' => '该查询无数据'), JSON_UNESCAPED_UNICODE);
        exit;
    }
    foreach ($arrayAll as $key_a => $info_a) {

        if ($arrayAll[$key_a]['bank_sign_up'] == 3) {
            echo json_encode(array('bank_sign_up' => $arrayAll[$key_a]['bank_sign_up'], 'msg' => '已签约'), JSON_UNESCAPED_UNICODE);
            exit;
        }
...
//    var_dump($data_array);
    if (strstr($data_array['info_list'][0]['msg'], '当日发送短信条数已达到上线')) {
        echo json_encode(array('bank_sign_up' => 13, 'msg' => '当日发送短信条数已达到上线'), JSON_UNESCAPED_UNICODE);
        exit;
    }
    ...
    if ($data_array['code'] == '00000000') {
        foreach ($data_array['info_list'] as $info_k => $info_v) {
            if ($data_array['info_list'][$info_k]['code'] == '00000000') {
                if (strstr($data_array['info_list'][$info_k]['msg'], '已签约')) {
                    $sqlSign = 'update base_bankcard set bank_sign_up=3 where member_id= ' . $_POST['member_id'] . ' and bank_num=' . $_POST['bank_num'];
                    $priceUseHy = $pdo->exec($sqlSign);
                    echo json_encode(array('bank_sign_up' => 3, 'msg' => '已签约'), JSON_UNESCAPED_UNICODE);
                    exit;
                }
                ...
                //添加日志文件
//                $logs = array(
//                    'withdraw_id' => $_POST['member_id'],
//                    'order_no' => $data_array['info_list'][$info_k]['order_no']
//                );
//                fwrite($fp_log_lyf, date("Y-m-d H:i:s") . "记录order_no" . json_encode($logs, JSON_UNESCAPED_UNICODE) . PHP_EOL);
//                发送签约申请成功 （修改为 bank_sign_up =2 签约中）
                if (!empty($data_array['info_list'][$info_k]['order_no'])) {
                    $priceUseHy = $pdo->exec('update base_bankcard set bank_sign_up=2, order_no=  ' . "'" . $data_array['info_list'][$info_k]['order_no'] . "'" . ' where member_id= ' . $_POST['member_id'] . ' and bank_num=' . $_POST['bank_num']);
                }
