...
include 'config.php';
//创建提现日志文件
$fp_log_lyf = fopen(dirname(__FILE__) . '/withdraw_batch_only.log', 'ab');
//数据库配置
//----------线上-----------****（清库时候要修改）
$GLOBALS['only'] = 'TJJZN1'; // 线上
$GLOBALS['num']  = 6; // $GLOBALS['only'] 为几位 num 就为几

// $GLOBALS['only'] = 'JJZN6'; // 线上
// $GLOBALS['num']  = 5; // $GLOBALS['only'] 为几位 num 就为几
//----------线上-----------****
//天溪app_id
    ...
    try {
        ...
        $sql = 'select
			   ...
			   b.bank_num,b.phone,b.real_name,b.id_card
				...
				where  w.status=4 and source_type=3 and b.is_del = 0 order by b.id asc limit 1';
        ...
        if (!$values) {
            ...
            // 修改处理中 status=5
            $sql             = 'update tx_live_withdraw w set   w.status= 5  where w.status=4 and w.source_type=3 and w.id=' . $values['withdraw_id'];
		 ...
		 values(' . $values['withdraw_id'] . ',' . $values['source_id'] . ',1' . ',' . "'" . '提现处理中' . "'" . ',' . time() . ')';
            ...
            $pdo->beginTransaction();
            //(代付接口结果返回处理) (//积分提现)
            //进行每一条提现记录判断 && ($data_array['code'] == '00000000' || $data_array['code'] == '00000018')
            if (!empty($data_array) && isset($data_array['code'])
                ...
                $priceUseHy = $pdo->exec('update tx_task_member set price_freeze=' . bcsub($member_credit['price_freeze'], $baseMemberSelect['credit'], 2) . ' where member_id= ' . $baseMemberSelect['member_id']);
                if ($priceUseHy == 0) {
                    echo "提现成功处理冻结积分出错：SQL-";
                    echo 'update tx_task_member set price_freeze=' . bcsub($member_credit['price_freeze'], $baseMemberSelect['credit'], 2) . ' where member_id= ' . $baseMemberSelect['member_id'] . PHP_EOL;
                    $pdo->rollBack();exit();
                }
                //
                ...
                $stmFreeze2->bindValue(':price', '-' . $baseMemberSelect['credit'], PDO::PARAM_STR);
            ...
            $withdrawUpdate = 'update tx_live_withdraw w ,tx_live_withdraw_log wl set  w.account_price= ' . $account_price . ' , w.status= ' . $status . ' , w.deal_time= ' . time() . '  where w.id=wl.withdraw_id and w.source_type=3 and w.status=5 and wl.id=' . $lastId;
