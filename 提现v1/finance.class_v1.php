...
b.id";
    ...
    public function withdrawal_list()
    {
        ...
        if($filter['tab']==0 || $filter['tab']==1){
            $sql = "select a.id,a.c_time,a.serial_number,b.login_name,b.nick_name,a.source_type,a.price,a.account_price,a.status,a.deal_time,a.member_id from tx_live_withdraw as a left join base_member as b on a.member_id = b.member_id where a.id > 0 {$where} {$status} order by a.c_time desc {$limit} ";
        }else{
            $sql = "select a.id,a.c_time,a.serial_number,b.login_name,b.nick_name,a.source_type,a.price,a.account_price,a.status,a.deal_time,a.member_id from tx_live_withdraw as a left join base_member as b on a.member_id = b.member_id where a.id > 0 {$where} {$status} order by a.deal_time desc {$limit} ";
        }
...
// 是否报错
    ...
    //提现批量改导出
    public function withdrawal_batch()
    {
        $id = ForceStringFrom('id');
//       echo $id;
        $arrayPost = array(
...
//        $redisWithdraw = $GLOBALS['redis']->lpush('withdraw_batch', json_encode($arrayPost));
        /**************************************放在redis队列中******************************/
        $url = $GLOBALS['API_URL_V2'] . '/withdraw_batch.php';
        $ch = curl_init();
        // 设置超时
