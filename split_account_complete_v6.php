...
//创建日志文件
echo $fp_log_lyf = fopen(dirname(__FILE__) . '/logs/split_account_complete_v6.log', 'ab');
...
do {
    ...
    /************************************************处理条数节点start********************/
    //查询处理到的节点判断
    $rowsMaxId = $pdo->query('select id_bonus_account from cms_bonus_account where is_calc=2 and is_need_calc=2 and reason=1 and     withdraw_status=1 and type in ' . $GLOBALS['split_type'] . ' order by id_bonus_account asc limit 1')->fetch(PDO::FETCH_ASSOC);

    if (empty($rowsMaxId)) {
        $rowsMaxId = $pdo->query('select id_bonus_account from cms_bonus_account where is_calc=2 and is_need_calc=2 and reason=1 and withdraw_status!=1 and type in ' . $GLOBALS['split_type'] . ' order by id_bonus_account desc limit 1')->fetch(PDO::FETCH_ASSOC);
...
//    原始SQL集合
    ...
    $sql = 'select 
            ...
            ba.id_user,ba.id_bonus_account,ba.is_calc,ba.type,ba.id_order,ba.withdraw_status,ba.credit,
            ...
            where ba.type in ' . $GLOBALS['split_type'] . ' and ba.withdraw_status=1 and ba.is_calc=2 and ba.is_need_calc=2 and ba.reason=1 and id_bonus_account between ' . $limitBefore . ' and ' . $limitAfter . ' order by ba.id_order asc ,ba.id_user asc ';
//    echo $sql;
    ...
    foreach ($arrayAll as $key_a => $info_a) {
        $resultAllSplit[$info_a['id_order']][$info_a['id_user']][] = $info_a;
    }
//    var_dump($resultAll);

    //计算相同id_order id_user credit的和
    foreach ($resultAllSplit as $key_order => $resultAll) {
        //id_order 层
        foreach ($resultAll as $key_r => $info_r) {
            ...
            $resultAllSplit[$key_order][$key_r]['credit_split_total'] = $item;
        }
    }

//    var_dump($resultAllSplit);
//exit;
    ...
    foreach ($resultAllSplit as $key_order => $resultAll) {
        ...
        foreach ($resultAll as $key_r_e => $info_r_e) {
            ...
            echo $creditSplitTotal[$key_r_e] . PHP_EOL; //需要分账的金额（数据库中）
            $valueChange['credit_split_total'] = bcmul($creditSplitTotal[$key_r_e], 100, 0);//需要分账的金额(分)
...
//            echo $type;
            ...
            try {
                ...
                if (!empty($basicInfo['wechat_openid']) || !empty($basicInfo['alipay_openid'])) {
                    ...
                    if ((int)$basicInfo['pay_type_sync'] == 1) {
                        ...
                        "account_list": [{
                                        "account": "' . $basicInfo['wechat_openid'] . '",
                                        "amount": ' . (int)$valueChange['credit_split_total'] . ',
                                        "description": "商品分享佣金",
                                        "relation_type": "1",
                                        "share_id": "' . $GLOBALS['only'] . $uniqueId . '",
                                        "share_name": "' . $basicInfo['wechat_name'] . '",
                        ...
                        if (!empty($basicInfo['alipay_openid'])) {
                            ...
                            $jsonAliSplit = '{
                                             ...
                                             "pay_type": "AliPay_App",
                                             "info_list": [{
                                                    "trade_no": "' . $basicInfo['pay_sn_sync'] . '",
            ...
            ##########################################插入cms_withdraw 和cms_withdraw_detail / cms_withdraw_detail表数据 start############
            try {
                $pdo->beginTransaction();
                if (!empty($type)) {
...
//                            var_dump($info_rr_e);
            ...
            ##########################################插入cms_withdraw 和cms_withdraw_detail cms_withdraw_detail表数据end#######################
...
//                        $stmFreeze2->bindValue(':memo', $data_array['data'][0]['msg'].'提现' . $basicInfo['pay_sn_sync'], PDO::PARAM_STR);
                        if (strstr($data_array['data'][0]['signsign'] , 'wx')!==false) {
                            if($data_array['data'][0]['signsign'] == 'wx_split'){
                                $stmFreeze2->bindValue(':memo', '微信分账成功:' . $data_array['data'][0]['msg'] . '提现'. $basicInfo['pay_sn_sync'], PDO::PARAM_STR);
                            }elseif($data_array['data'][0]['signsign'] == 'wx_transfer'){
                                $stmFreeze2->bindValue(':memo', '微信转账成功:' . $data_array['data'][0]['msg'] . '提现'. $basicInfo['pay_sn_sync'], PDO::PARAM_STR);
                            }
                        } elseif (strstr($data_array['data'][0]['signsign'] , 'ali')!==false) {
                            if($data_array['data'][0]['signsign'] == 'ali_split'){
                                $stmFreeze2->bindValue(':memo', '支付宝分账成功:' . $data_array['data'][0]['msg'] . '提现'. $basicInfo['pay_sn_sync'], PDO::PARAM_STR);
                            }elseif($data_array['data'][0]['signsign'] == 'ali_transfer'){
                                $stmFreeze2->bindValue(':memo', '支付宝转账成功:' . $data_array['data'][0]['msg'] . '提现'. $basicInfo['pay_sn_sync'], PDO::PARAM_STR);
                            }
                        }
...
//                        print_r($data_array['data'][0]['msg']);
                        ...
                        foreach ($info_r_e as $key_rr_e => $info_rr_e) {
                            if ($key_rr_e !== 'credit_split_total') {
                                $noAccountUpdate = 'update cms_bonus_account ba  set  withdraw_status= ' . $withdraw_status . ' where id_bonus_account=' . $info_rr_e['id_bonus_account'];
                                $accountBonusUpdate = $pdo->exec($noAccountUpdate);
                            }
                        }
//                需要将cms_credit 表中的数据freeze还原（锁住）
