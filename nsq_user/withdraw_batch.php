...
//创建提现日志文件
$fp_log_lyf = fopen(dirname(__FILE__) . '/withdraw_batch.log', 'ab');
//数据库配置
...
do {
    try {
        ...
        $redisWithdraw = $GLOBALS['redis']->lpop('withdraw_batch');
        if ($redisWithdraw) {
            $redisData = json_decode($redisWithdraw, true);
            var_dump($redisData);
            withdrawBatch($pdo, $redisData,$fp_log_lyf); //批量(选中)
        } else {
            $pong = $GLOBALS['redis']->ping();
            if ($pong != 'pong') {
                echo "Redis 连接失败";
            }
            echo '无要处理的redis队列' . PHP_EOL;
...
//            var_dump(explode(':',$creditConfigSelect['name_com']));
            ...
            $Sign = $GLOBALS['redis']->lpush('withdraw_sign', json_encode(array('sign' => 1)));
            $redisSgin = json_decode($Sign, true);
            var_dump($redisSgin);
        }
//        echo '代码底层';
        echo $returnReturn;
